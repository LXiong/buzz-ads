...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			appendTitle: false, //table caption text is added to chart
			title: null, //grabs from table caption if null
			appendKey: false, //color key is added to chart
			colors: ['#FF9900','#3191D6','#92d5ea','#ee8310','#8d10ee','#5a3b16','#26a4ed','#f45a90','#e9e744'],
			...
			autoYWidth: false, // auto scales the Y axis padding to fit large numbers. If multiple charts on a page or a fixed width chart, user may not want this.
			isRound:true, // display date is round 
			symbol:"¥", //set money symbol
			unit:""
		},options);
		...
		tableData.dataSum = 0;
		tableData.topValue = 1;
		tableData.bottomValue = 0; // this defaults to 0
		...
		if($.isFunction(o.xLabelParser)) {
			...
			var marginDiffX = 0;
			if(o.lineMargin) {
				marginDiffX = -2*xScale-o.lineMargin;
			}
		...
		var numLabels = Math.floor((o.height - 2*o.lineMargin) / o.yLabelInterval);
		if (o.yLabelNumber) numLabels = o.yLabelNumber;
				...
				setup: function(area){
					...
					if(!o.customXLabels) {
						...
						$.each(xLabels, function(i){ 
							...
							var leftOffset = label.width()*(3/4)/(i === xLabels.length-1 ? -1 : -2);
							label
								.css({'margin-left': leftOffset})
								.addClass('label');
							if (i != xLabels.length-1 && i % o.xLabelInterval != 0) {
								thisLi.hide();
							}
							if(i==xLabels.length-1 && !o.isRound){
								thisLi.hide();
							}
							if (!o.showXLines) thisLi.find('span.line').hide();
						});
					} else {
					...
					$.each(yLabels, function(i){
						...
						// adjust the left side padding for large numbers:
						if ($.browser.msie) {
							$("#" + o.chartId + " .visualize-labels-x").css({
								left: 0,
								top: 0
							});
						} else {
							if (i == yLabels.length-1 && o.chartId && o.autoYWidth) {
								// this should be the largest value
								var addPad = Math.round(value / 100) + "";
								addPad = (addPad.length - 1) * 8;
								$("#" + o.chartId).css("padding-left",30+addPad+"px");
								$("#" + o.chartId + " .visualize-labels-x").css("left",30+addPad+"px");
								$("#" + o.chartId + " .visualize-labels-y").css("left",30+addPad+"px");
							}
						}
						if(posB > o.height || posB < 0) {
							return;
						}
						var thisLi = $('<li><span>'+addCommas(value)+'</span></li>')
							.css('bottom', posB);
						...
						var label = thisLi.find('span:not(.line)');
						var topOffset = -6;//label.height()/-2;
						//if(!o.lineMargin) {
		...
		if( typeof(window.G_vmlCanvasManager) !== 'undefined' ){ window.G_vmlCanvasManager.init(); window.G_vmlCanvasManager.initElement(canvas[0]); }	
		...
		if(!container){
			...
			self.bind('visualizeRedraw', function(){
				if (o.type === 'area') charts[o.type].draw(true);
				else charts[o.type].draw();
			});
		}
