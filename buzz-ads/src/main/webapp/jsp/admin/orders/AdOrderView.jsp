...
<div class="container-body">
    ...
    <div>
        ...
        <div class="container-right" style="overflow: auto;">
            <div class="warningMessage div-rounded-5" style="display:none"><p>广告活动的投入时间变更为   <span class="orange" style="padding:0px 3px;">${request.adCampaign.startDateStr } -  
                    <s:if test='%{request.adCampaign.endDateStr== null || request.adCampaign.endDateStr  == ""} '>
                        无截止日期
                    </s:if>
                    <s:else>
                        ${request.adCampaign.endDateStr }
                    </s:else>
            ...
            <div class="overview bc">
                <p class="breadcrumb">
                    ...
                    &gt;&gt;
                    <a class="bLinkD" title="<s:property value="%{#request.adCampaign.name}" escape="false"/>"  href="${cxt_path}/admin/campaigns/view?id=${adCampaign.id}">广告活动  - <core:partOfText value="${adCampaign.name}" textmaxlength="30" escape="false"/></a>
                        ...
                        <td>
                            <div class="">
                                ...
                                <span id="displayBidType" style=" margin: 0px 50px 0px 0px;">
                                    <s:if test="#request.adCampaign.bidType == (@com.buzzinate.buzzads.enums.BidTypeEnum@CPC)">
                                        CPC
                                    </s:if><s:else>
                                        CPM
                                    </s:else>
...
<script type="text/javascript">
    $(function(){
        ...
        function diffDate(){
            var cas="${request.adCampaign.startDateStr }";
            var cae="${request.adCampaign.endDateStr }";
        ...
        function init(){
            ...
            if("${adCampaign.maxDayBudgetDouble }"== '0.00'){
                $("#putInTypeCon").hide();
        ...
        $("#now").click(function(){
            if($(this)[0].checked){
                if("${adCampaign.startDateStr}" == ""){
                    var d=new Date();
                    date=d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate();
                } else {
                    date = new Date("${adCampaign.startDateStr}");
                }
       ...
       $("#specificTimes").click(function(){
        ...
        function submit(isSave){
            ...
            if(dataTemp.isBid && dataTemp.isReady && dataTemp.isStart && dataTemp.isFrequencyCap && dataTemp.isPutInWay && dataTemp.isName  && dataTemp.isWeek && dataTemp.isHours){
                $.ajax({
                    type: "post",
                    url: "${cxt_path}/admin/orders/update",
                    data:dataTemp,
                    dataType:"json",
                    success: function (data) {
                        if (data.success) {
