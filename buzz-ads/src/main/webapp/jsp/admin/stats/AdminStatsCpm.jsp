    ...
    <div class="container-right" style="overflow:auto;">
        <div class="overview">
        	<div class="left topUp">
	                <span>正在投放的CPM广告活动数量：${cpmCount }</span>
	        </div>
	        <div class="left" style="margin-left:40px; border-left: 1px solid #dadada; padding-left: 40px;">
				<table class="bTableInfo"  style="font-size:14px;" >
					<tbody>
						<tr>
							<td>上月的收入</td><td><font class="unit">¥</font><font class="number"> ${lastTotalStats.cpmTotalCommissionDouble}</font></td><td></td>
						</tr>
						<tr>
							<td >本月的收入</td><td><font class="unit">¥</font><font class="number"> ${currentTotalStats.cpmTotalCommissionDouble}</font></td>
							 <td >
        ...
        <div class="overviewPanel shadow">
	            <div class="dataSearch formDate">
	                 <form action="cpm" method="post" id="overviewForm">
	                     <div class="left">
        ...
        <div class="overview" style="border-top:0px;" >
		        ...
		        <table class="bTable textRight">
                    <tr class="heading">
                        ...
                        <th class="textRight">
                            <s:if test="#request.sortColumn eq 'cpmViews'">
                                <a class="sort" href="javascript:sortOperate('cpmViews', <s:if test='#request.sequence eq "ASC"'>'DESC'</s:if><s:else>'ASC'</s:else>)">
                                                                                                            有效展示量<span class="<s:if test='#request.sequence eq "ASC"'>sort_up_select</s:if><s:else>sort_down_select</s:else>"></span></a>
                            </s:if>
                            <s:else>
                                <a class="sort" href="javascript:sortOperate('cpmViews', 'DESC')">有效展示量<span class="sort_down"></span></a>
                            </s:else>                
                        </th>
                        <th style="text-align:center">
	                        <s:if test="#request.sortColumn eq 'cpmCommission'">
	                            <a class="sort" href="javascript:sortOperate('cpmCommission', <s:if test='#request.sequence eq "ASC"'>'DESC'</s:if><s:else>'ASC'</s:else>)">
	                        ...
	                        <s:elseif test="#request.sortColumn eq ''|| #request.sortColumn == null ">
	                            <a class="sort" href="javascript:sortOperate('cpmCommission', 'ASC')">收入<span class="sort_down_select"></span></a>
	                        </s:elseif>
	                        <s:else>
	                            <a class="sort" href="javascript:sortOperate('cpmCommission', 'DESC')">收入<span class="sort_down"></span></a>
	                        </s:else>
                        </th>
                    </tr>
                    <s:if test="#request['stats'].size() > 0">
                            <s:iterator value="#request['stats']" var="stat" status="status" >
                                <tr <s:if test="%{#status.odd}">style="background:#F2F6F9;"</s:if>>
                                    <td style="text-align:left;"><a class="bLink orange" href="${cxt_path }/admin/campaign/${campaignId}/detail?leftmenu=leftmenu-admin-stats-cpm">${stat.campName}</a></td> 
                                    ...
                                    <td><s:text name="global.format.number"><s:param value="%{#stat.cpmViewNo}"/></s:text></td>
                                    <td style="text-align:center">¥${stat.cpmTotalCommissionDouble}</td>
                                </tr>
                            </s:iterator>
                     </s:if>
                     <s:else>
