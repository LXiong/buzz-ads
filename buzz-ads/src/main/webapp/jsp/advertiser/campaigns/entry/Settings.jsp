...
<title>设置 - ${adEntry.name} - ${selectedGroup.name} - ${selectedCampaign.name} - BuzzAds</title>
...
<div class="container-body">
	...
	<div>
		...
		<div class="container-right" style="overflow: auto;">
            <div class="warningMessage div-rounded-5"><p>修改广告项，广告将重新审核，请耐心等待。</p></div>
            <div class="clear spacer10"> </div>
            <div class="overview bc">
				<p class="breadcrumb">
					<a class="bLinkD" style="padding-left:15px;" href="${cxt_path}/advertiser/campaigns">全部活动</a>
					&gt;&gt;
					<a class="bLinkD" href="${cxt_path}/advertiser/campaign/${selectedCampaign.id}/groups">广告活动  - <core:partOfText value="${selectedCampaign.name}" textmaxlength="30" escape="true"/></a>
					&gt;&gt;
					<a class="bLinkD" href="${cxt_path}/advertiser/group/${selectedGroup.id}/entries">广告组 - <core:partOfText value="${selectedGroup.name}" textmaxlength="30" escape="true"/></a>
					&gt;&gt;
					<a class="orange" href="${cxt_path}/advertiser/group/${adEntry.name}/entries">广告 - <core:partOfText value="${adEntry.name}" textmaxlength="30" escape="true"/></a> 
				</p>
			</div>
			<div class="clear spacer10"> </div>
			...
			<div style="width:770px" class="navCon left">
                <table class="viewTable" style="width:100%">
                    ...
                    <s:hidden name="entryId" id="entryId" value="%{#request.adEntry.id}" />
                </table>
			</div>
            <div class="left">
                <div class="spacer20"></div>
...
<script type="text/javascript">
	$(function(){
		//set paramete
    	var dataTemp={};
    	dataTemp.campaignId=${selectedCampaign.id};
		dataTemp.orderId=${selectedGroup.id};
		dataTemp.entryId=${request.adEntry.id}
		//更改活动状态
			 ...
			 new AjaxUpload("#imageFileUpload", {
		            ...
		            onComplete : function(file, response) {
		                if (response.success) {
		                   ...
		                   // $("#imgDisplay").attr("src", response.contents.resourceUrl);
		                    var size=($("#sizeSelector").val()).match(/\d+/g);
    	...
    	*/
		...
		function submit(isSave){
			...
			if (dataTemp.isName && dataTemp.isHeadline && dataTemp.isImgUrl && dataTemp.isDesturl && dataTemp.size) {
				$.ajax({
					type: "post",
					url: "${cxt_path}/advertiser/entry/editAction",
					data:dataTemp,
					dataType:"json",
					success: function (data) {
						if(data.success) {
	                       	if(isSave){
	                       		displayStatusMessage("保存成功！", "success"); 
		                       	 window.setTimeout(function(){
		                             window.location="${cxt_path}/advertiser/group/"+${selectedGroup.id}+"/entries";  
			                        }, 50);	
		                       	}else{
		                       		displayStatusMessage("保存成功并继续新建广告！", "success"); 
		                       		window.setTimeout(function(){
			                            window.location="${cxt_path}/advertiser/entry/new?campaignId="+ ${selectedCampaign.id} +"&orderId="+${selectedGroup.id};  
			                        }, 50);	
		                       	}						
							} else {
