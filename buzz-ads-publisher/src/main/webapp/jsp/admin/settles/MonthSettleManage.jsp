		...
		<div class="container-right" style="overflow:auto;">
			<div class="overview">
				<table class="bTableInfo"  style="font-size:14px;" >
	            	<tr>
	                    <td>结算月</td>
	                    <td style="padding-left:0px;"><font class="number"><s:date name="#request.settleMonth" format="yyyy-MM"/></font></td>
	                    <td></td>
	                </tr>	
	                <tr>
	                    <td>总佣金</td>
	                    <td style="padding-left:0px;"><font class="unit">¥</font><font class="number"> ${totalCommissionDouble}</font></td>
	                    <td>
	                    	<s:if test="%{#request.currentPageRole == 'admin'}">
						        <div><a class="bLink orange spacer_left50"  href="${cxt_path}/settlement/payments">查看支付历史</a></div>
						    </s:if>
						    <s:if test="%{#request.currentPageRole == 'finance'}">
				                <div><a class="bLink orange spacer_left50"  href="${cxt_path}/finance/settles/payments">查看支付历史</a></div>
				            </s:if>
	                    </td>
	                </tr>
	            </table>
			...
			<div class="overviewPanel shadow">
				<div class="dataSearch" style="line-height:25px;">
			        ...
			        <span class="left" style="top: 4px;position: relative;">结算月：</span>
			        <input id="settleMonthPicker" style="width:100px;text-align:left;" class="bTextbox left" type="date"
				...
				<table class="bTable">
					...
					<s:if test="#request['monthSettles'].size() > 0">
						...
						<tr <s:if test="%{#status.odd}">style="background:#F2F6F9;"</s:if>>
							...
							<td style="text-align:center;">${iterObj.receiveBankName}</td>
							<td class="textRight">¥${iterObj.commissionDouble}</td>
							<td style="text-align:right;">
								<s:if test="%{#request.currentPageRole == 'admin'}">
								    <a class="bLink" href="${cxt_path}/admin/settles/detail?userId=${iterObj.userId}">查看</a>&nbsp;
								</s:if>
								<s:if test="%{#request.currentPageRole == 'finance'}">
...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
	    ...
	    $("#calmonth").add("#calyear").live("change",function(){
