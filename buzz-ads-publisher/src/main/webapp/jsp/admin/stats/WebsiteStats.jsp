...
<div class="container-body">
	...
	<div>
		...
		<div class="container-right" style="overflow:auto;">
			 <form action="${cxt_path}/admin/stats/website" method="post" id="filterSearchForm" name="filterSearchForm">
			 <div class="overviewPanel shadow">
			 	<div class="dataSearch right" >
	                ...
	                <input class="bButton bButton24 left mSpacer_left10"  type="submit" style="width:90px;" onclick="doSearch()" value="查询" />     
                </div>
			 </div>
			...
			<div class="overviewPanel details" style="border-top:0px;">
				<div class="left tabTitle">
		        	...
		        	<a class="  mSpacer_left10" href="${cxt_path}/admin/stats/time"> 按时间</a>
		         	...
		         	<a class=" mSpacer_left10 orange" href="javascript:void(0)" style="cursor:default;">按网站</a>	
		        </div> 
		        <div class="right mSpacer_left10" style="top: 3px;position: relative;">
		        	<div class="left" style="line-height:32px;height:32px;font-size:14px;">网站地址</div>
		        	<input type="text" class="bTextbox mSpacer_left10  left" style="width:auto;" placeholder="Search by website..."  id="siteName" name="siteName" value="${siteName}"/>
		        </div>
			</div>
		</form>
		   	<div class="overview" style="border-top:0px;">
		        ...
		        <table class="bTable textRight">
		        	...
		        	<s:if test="#request.siteStats != null && #request['siteStats'].size() > 0">
                        <s:iterator value="#request['siteStats']" var="stat" status="status">
                            <tr <s:if test="%{#status.odd}">style="background:#F2F6F9;"</s:if>>
                                <td style="text-align:left;" title="<s:property value="%{#stat.siteName}" escape="true"/>"><core:partOfText value="${stat.siteName}" textmaxlength="30" escape="true"/></td>
...
function doSearch() {
	$("#sWebsite").val($.trim($("#sWebsite").val()));
