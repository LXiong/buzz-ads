...
public class UpYunDemo {
	public static void main(String[] args) throws Exception{
		/// 初始化空间
		UpYun upyun = new UpYun("lezhi", "buzzinate", "buzzinate");
		...
		File file2 = new File("D:/Temp/test2.jpg");
		
		/// 设置待上传文件的 Content-MD5 值（如又拍云服务端收到的文件MD5值与用户设置的不一致，将回报 406 Not Acceptable 错误）
		upyun.setContentMD5(UpYunUtil.md5(file));
		...
		upyun.setContentMD5(UpYunUtil.md5(file2));
		System.out.println(upyun.writeFile("/test2.jpg", file2, true));
		// System.out.println(upyun.writeFile("/a/b/c/google.jpg", file, true)); //可自动创建父级目录（最多10级）
		/// 获取上传文件后的信息（仅图片空间有返回数据）
		System.out.println(upyun.getUrl("/test.jpg"));
