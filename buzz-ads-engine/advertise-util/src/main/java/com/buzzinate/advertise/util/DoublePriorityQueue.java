...
public class DoublePriorityQueue<E> {
	...
	public static void main(String[] args) {
		DoublePriorityQueue<Integer> pq = new DoublePriorityQueue<Integer>(4, ASC);
	...
	public DoublePriorityQueue(int maxSize, Integer order) {
		size = 0;
	...
	private boolean compare(Entry<E> a, Entry<E> b) {
		if(order == ASC){
	...
	public final Entry<E> add(double key, E value) {
		...
		if (size < maxSize) {
			...
			upHeap();
			return null;
		} else if (size > 0 && !compare(e, heap[1])) {
			Entry<E> ret = heap[1];
			heap[1] = e;
			updateTop();
			return ret;
	...
	private final void downHeap() {
		...
		int k = j + 1;
		if (k <= size && compare(heap[k], heap[j])) {
			j = k;
		}
		while (j <= size && compare(heap[j], node)) {
