...
object TestLda {
  def main(args: Array[String]): Unit = {
    ...
    val source = CSVFile("/home/feeling/corpus.csv") ~> IDColumn(1)
    ...
    for (numTopics <- 30 until 100 by 10) {
      ...
      try {
        TrainGibbsLDA(params, dataset, output = gibbsModelPath, maxIterations = 1000);
      } catch {
